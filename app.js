...
function announceEvent(event, team) {
    if (event === 'MATCH') {
        speak(`Juego, set y partido para ${getTeamName(team)}`);
    } else if (event === 'SET') {
        const currentSetsT = state.sets[team];
        const currentSetsO = state.sets[team === 'A' ? 'B' : 'A'];
        speak(`Juego y set para ${getTeamName(team)}. ${currentSetsT} set a ${currentSetsO}.`);
    } else if (event === 'GAME') {
        const gamesT = state.games[team];
        const gamesO = state.games[team === 'A' ? 'B' : 'A'];
        speak(`Juego para ${getTeamName(team)}. ${gamesT} a ${gamesO} en juegos.`);
    } else {
        const pointText = getPointText();
        // Prevent redundant "Point for A. Advantage A."
        if (pointText.includes('Ventaja')) {
            speak(pointText);
        } else {
            speak(`Punto para ${getTeamName(team)}. ${pointText}`);
        }
    }
}
...
function askScore() {
    if (state.matchOver) {
        speak(`El partido ha terminado. Ganó ${getTeamName(state.winner)}`);
        return;
    }
...
function handleVolumeChange() {
    ...
    const now = Date.now();
    if (now - lastVolTime < 100) return; // Prevent rapid spam
    lastVolTime = now;
...
document.addEventListener('keydown', (e) => {
    ...
    const isTrigger = allTriggers.includes(e.key) || e.keyCode === 13 || e.keyCode === 175 || e.code === 'VolumeUp';
    if (isTrigger) {
        e.preventDefault();
...
initOverlay.addEventListener('click', () => {
    ...
    // Apple Watch / Fallback Volume Hook Media Loop
    try {
        ...
        silentAudio.volume = 0.5; // Enable volume change capturing natively
